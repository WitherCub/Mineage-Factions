	...
	public void invoke(long l)
	{
		...
		for (UUID uuid : CmdFactionsShield.get().playersWithShieldGuiOpen)
		{
			...
			if (faction.getShieldedHoursCooldownFromDisable() != null)
			{
				...
				inventory.setItem(39, new ItemBuilder(Material.WATCH).name(Txt.parse(MConf.get().pendingChangeGuiName)).setLore(MConf.get().pendingChangeGuiLoreChangeAfterDisable.stream().map(s -> Txt.parse(s.replace("%time%", timeRemaining))).collect(Collectors.toList())));
			}
			else if (faction.getShieldedHoursChangeRequestNewStartTime() == null)
			{
				inventory.setItem(39, new ItemBuilder(Material.WATCH).name(Txt.parse(MConf.get().pendingChangeGuiName)).setLore(MConf.get().pendingChangeGuiLoreNoChange.stream().map(Txt::parse).collect(Collectors.toList())));
			}
			else
			{
