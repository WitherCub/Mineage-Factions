...
import com.massivecraft.factions.cmd.CmdFactionsLeave;
...
public class CmdFactionsAltjoin extends FactionsCommand {
	...
	public void perform() throws MassiveException {
		...
		if (MConf.get().altLimit > 0 && alts >= MConf.get().altLimit) {
			msg(" <b>!<white> The faction %s is at the limit of %d alts, so %s cannot currently join.", faction.getName(msender), MConf.get().altLimit, mplayer.describeTo(msender, false));
			return;
		}
		...
		if (!(faction.isAltsOpen() || faction.isInvitedAlt(mplayer) || msender.isOverriding())) {
