...
public class CmdFactionsAltsRevoke extends FactionsCommand
{
	private static CmdFactionsAltsRevoke i = new CmdFactionsAltsRevoke();
	public static CmdFactionsAltsRevoke get() { return i; }
	
	public CmdFactionsAltsRevoke()
	{
	...
	public void perform() throws MassiveException
	{
		...
		if ("all".equalsIgnoreCase(this.argAt(0)))
		{
			...
			{
				throw new MassiveException().addMsg("<b>No one is invited to your faction as an alt.");
			}
			all = true;
		...
		for (MPlayer mPlayer : mplayers)
		{
			...
			{
				msender.msg("<b>%s is invited as a member. Please use /f deinvite to revoke this invitation.", mPlayer.getName());
		...
		for (MPlayer mplayer : mplayers)
		{
			...
			{
				// Mson
				String command = CmdFactionsAltsInvite.get().getCommandLine(mplayer.getName());
				String tooltip = Txt.parse("Click to <c>%s<i>.", command);
