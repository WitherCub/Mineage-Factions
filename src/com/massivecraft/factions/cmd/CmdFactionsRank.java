...
{
	private static CmdFactionsRank i = new CmdFactionsRank();
	public static CmdFactionsRank get() { return i; }
	// -------------------------------------------- //
	// CONSTANTS
	// -------------------------------------------- //
	...
	private void ensureAllowed() throws MassiveException
	{
		...
		if (target.isAlt())
		{
			// You can't change the rank of alts.
			throw new MassiveException().addMsg("<b>You can't change the rank of alts.");
		}
	}
	...
	{	
		...
		// Event
		EventFactionsMembershipChange membershipChangeEvent = new EventFactionsMembershipChange(sender, msender, endFaction, MembershipChangeReason.RANK, false);
	...
	private void changeRankLeader()
	{
		...
		// Clear new leaders pperms
		target.clearPlayerPerms();
