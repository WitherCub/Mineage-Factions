...
public class CmdFactionsJoin extends FactionsCommand
{
	...
	public void perform() throws MassiveException
	{
		...
		{
			String command = CmdFactionsKick.get().getCommandLine(mplayer.getName());
		...
		if (!mplayer.isOverriding() && MConf.get().factionMemberLimit > 0 && (faction.getMPlayers().size() - alts) >= MConf.get().factionMemberLimit)
		{
		...
		{
			String command = CmdFactionsLeave.get().getCommandLine(mplayer.getName());
		...
		if (msenderFaction.isPlayerBanned(mplayer.getPlayer()))
		{
			msg("<b>You are currently banned from this faction and cannot join.");
			return;
		}

		if( ! (faction.getFlag(MFlag.getFlagOpen()) || faction.isInvited(mplayer) && !faction.isInvitedAlt(mplayer) || msender.isOverriding()))
		{
		...
		// Event
		EventFactionsMembershipChange membershipChangeEvent = new EventFactionsMembershipChange(sender, msender, faction, MembershipChangeReason.JOIN, false);
...
//		mplayer.setPowerBoost(faction.getLevel(MissionUpgradeConf.get().powerboostUpgrade.getUpgradeName()) * 10D);
