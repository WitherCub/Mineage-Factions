...
public class EngineMoveChunk extends Engine
{
	...
	public void moveChunkDetect(PlayerMoveEvent event)
	{
		...
		// ... update fly
		if (MConf.get().factionsFlyEnabled)
		{
			EngineFly.get().chunkChangeFlight(mplayer, player, chunkFrom, chunkTo);
		}
		
		// ... check if inspecting and if they can inspect there
		if (!MPerm.getPermInspect().has(mplayer, BoardColl.get().getFactionAt(chunkTo), false) && mplayer.isInspecting()) {
			mplayer.setInspecting(false);
			mplayer.msg("<n>You have entered land that you do <b>not have permission <n>to inspect, <b>disabling <n>block inspection.");
		}
	}
