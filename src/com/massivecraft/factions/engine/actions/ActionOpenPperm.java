...
public class ActionOpenPperm extends ChestActionAbstract
{
	
	private String mPerm;
	private MPlayer mPlayer;
	
	public ActionOpenPperm(String mPerm, MPlayer mPlayer)
	{
		this.mPerm = mPerm;
	...
	public boolean onClick(InventoryClickEvent event)
	{
		...
		ChestGui gui = ChestGui.getCreative(Bukkit.createInventory(null, InventoryType.HOPPER, ChatColor.RED + "Editing " + mPerm.getName() + "..."));
		gui.setAutoclosing(true);
		gui.setAutoremoving(true);
		gui.setSoundOpen(null);
		gui.setSoundClose(null);
		
		Boolean status = mPlayer.getPlayerPermValue(mPerm);
		
		// slot 0
		ItemStack toggleYes = new ItemStack(Material.STAINED_GLASS_PANE, 1);
		ItemMeta yesMeta = toggleYes.getItemMeta();
		
		yesMeta.setDisplayName(ChatColor.GREEN + "Click to set pperm for " + mPlayer.getName());
		...
		gui.setAction(0, new ActionPpermModify(faction, mPlayer, true, this.mPerm));
		
		// slot 2
		ItemStack toggleNo = new ItemStack(Material.STAINED_GLASS_PANE, 1);
		ItemMeta noMeta = toggleNo.getItemMeta();
		
		noMeta.setDisplayName(ChatColor.RED + "Click to set pperm for " + mPlayer.getName());
		...
		gui.setAction(2, new ActionPpermModify(faction, mPlayer, false, this.mPerm));
		
		// slot 4
		ItemStack unset = new ItemStack(Material.STAINED_GLASS_PANE, 1);
		ItemMeta unsetMeta = unset.getItemMeta();
		
		unsetMeta.setDisplayName(ChatColor.YELLOW + "Click to unset pperm for " + mPlayer.getName());
		...
		gui.setAction(4, new ActionPpermModify(faction, mPlayer, null, this.mPerm));
		// end slots
